...
class Entity:
    ...
    def is_hidden(self):
        """
        Whether an object is ignored during rendering
        """
        pass
...
class Box(Entity):
    ...
    def __init__(self, color, size=1.0, is_hidden=False):
        super().__init__()
...
class Ball(MeshEnt):
    ...
    def __init__(self, color, size=1, is_hidden=False):
        assert color in COLOR_NAMES
        super().__init__(
...
class Cone(MeshEnt):
    def __init__(self, size=1.2, is_hidden=False):
        super().__init__(
            mesh_name='cone',
            height=size,
            static=False
